...
export class ValidationService {
  ...
  public validateTime(startDate: Date, endDate: Date,
    ...
    if (startDate && endDate && startDate.valueOf() - endDate.valueOf() === 0 && startTime && endTime &&
      ((startTime.minute > endTime.minute && startTime.hour === endTime.hour) || startTime.hour > endTime.hour)) {
      return true;
    } else if (startTime && startTime.hour && startTime.minute && endTime && endTime.hour && endTime.minute
